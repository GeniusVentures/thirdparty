...
cd $X86_64_DIR
if [ ! -f "Makefile" ]; then
  $SRC_DIR/Configure $DEBUG_FLAGS darwin64-x86_64-cc -static --prefix=$LIB_DIR --openssldir=$X86_64_DIR
fi
...
cd $ARM64_DIR
if [ ! -f "Makefile" ]; then
  $SRC_DIR/Configure $DEBUG_FLAGS darwin64-arm64-cc no-asm -static --prefix=$LIB_DIR --openssldir=$ARM64_DIR
fi
